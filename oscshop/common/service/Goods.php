...
class Goods
{
    ...
    public function ajax_get_goods($page_num, $limit_num)
    {
        ...
        $sql = 'SELECT goods_id,periods,image,price,name FROM ' . config('database.prefix') . 'goods WHERE status=1 ORDER BY goods_id LIMIT ' . $limit;
    ...
    public function get_goods_info($goods_id)
    {
        ...
        if (!$goods = Db::name('goods')->alias('g')->join('__GOODS_DESCRIPTION__ gd',
            'g.goods_id = gd.goods_id')->where('g.goods_id', $goods_id)->find()) {
            return false;
        }
        return [
            'goods' => $goods,
            ...
            'mobile_description' => Db::name('goods_mobile_description_image')->where('goods_id',
                $goods_id)->order('sort_order asc')->select()
        ];
    }
