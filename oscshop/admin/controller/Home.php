...
use think\Db;
use osc\admin\model\Home as home_model;
...
class Home extends AdminBase
{
    ...
    {
        parent::_initialize();
        $this->assign('breadcrumb1', '房间');
        $this->assign('breadcrumb2', '房间管理');
    }

    public function arithmetic()
    {
        $lottery_timestamp = time();//时间戳
        $lottery_num = 2405;//中奖号码
        $lottery_drifts = 100;//份额

        $suiji_num = re_arithmetic($lottery_timestamp, $lottery_num, $lottery_drifts);
        dump($lottery_timestamp);
        dump($lottery_num);
        dump($lottery_drifts);
        dump($suiji_num);
        $z = arithmetic($lottery_timestamp, $lottery_drifts);
        dump($z);
        exit;
    }
    ...
    public function index()
    {
        ...
        $list = (new home_model())->get_home_list();
    ...
    public function edit_discount()
    {
        ...
        $this->assign('goods_discount',
            Db::name('goods_discount')->where('goods_id', input('id'))->order('quantity ASC')->select());
    ...
    {
        $this->assign('goods_images',
            Db::name('goods_image')->where('goods_id', input('id'))->order('sort_order asc')->select());
    ...
    {
        $this->assign('mobile_images', Db::name('goods_mobile_description_image')->where('goods_id',
            input('id'))->order('sort_order asc')->select());
    ...
    function ajax_eidt()
    {
        if (request()->isPost()) {
            ...
            if (isset($data['id']) && $data['id'] != '') {
                ...
                if ($r) {
                    storage_user_action(UID, session('user_auth.username'), config('BACKEND_USER'),
                        '更新商品' . $data['id']);
                    return ['success' => '更新成功'];
                } else {
                    return ['error' => '更新失败'];
